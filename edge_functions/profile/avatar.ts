...
export const profileApi_changeAvatarUrl = async (
  req: NextApiRequest,
  res: NextApiResponse,
  ...
  const { avatar_url } = JSON.parse(req.body);

  // Handle no data
  if (!avatar_url) return res.status(400).json({ error: true, message: 'No data', data: null });
